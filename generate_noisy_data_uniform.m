...
% Set noise intensity
eta = 0.01;  % Small measurement noise

% Generate measurement noise (uniform in [-1,1])
measurement_noise_x = eta * (2 * rand(size(sol,1), 1) - 1);
measurement_noise_y = eta * (2 * rand(size(sol,1), 1) - 1);
measurement_noise_z = eta * (2 * rand(size(sol,1), 1) - 1);
...
disp('Stochastic data generation complete: Measurement noise added to Lorenz data.');
