...
eta_x = eta; % Noise coefficient for x
eta_y = eta; % Noise coefficient for y
eta_z = eta; % Noise coefficient for z
...
dW_x = eta_x * (2 * rand(size(tspan)) - 1); % Uniform noise in [-1,1]
dW_y = eta_y * (2 * rand(size(tspan)) - 1);
dW_z = eta_z * (2 * rand(size(tspan)) - 1);
